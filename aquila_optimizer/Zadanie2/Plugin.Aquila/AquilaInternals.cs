    ...
    double r1 = 10)
{
    ...
    private readonly double _sigma = SpecialFunctions.Gamma(1.0 + beta) * double.Sin(double.Pi * beta / 2.0) /
                                     (SpecialFunctions.Gamma((1.0 + beta) / 2.0) * beta *
                                      double.Pow(2.0, (beta - 1.0) / 2.0));
    ...
    public List<double> ExpandedExploitation(List<double> xBest, List<double> xMean, double[][] domain)
    {
        var a1 = _random.NextDouble();
        var a2 = _random.NextDouble();
        return xBest.Zip(xMean).Select((
                (double best, double mean) x, int index) => (x.best - x.mean) * alpha - a1 +
                                                            ((domain[index][1] - domain[index][0]) * a2 +
                                                             domain[index][0]) * delta
        ).ToList();
    }
    ...
    public List<double> NarrowedExploitation(List<double> xBest, List<double> xPrev, double t, double T)
    {
        ...
        return xBest.Zip(xPrev)
            .Select(((double best, double prev) x) => qf * x.best - g1 * x.prev * a1 - g2 * a2 + a3 * g1).ToList();
    }
}